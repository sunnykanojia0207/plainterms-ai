/**
 * DEMO FIXTURE V1 — the "before" version of the sample service agreement.
 * Original synthetic text for development and demos. NOT a real contract.
 *
 * Deliberate differences vs V2 (lib/documents/fixture.ts):
 * - Payment: net 15 (V1) vs net 30 (V2) + V1 has a 1.5%/month late fee V2 drops
 * - Termination notice: 15 days (V1) vs 30 days (V2)
 * - IP transfer: on each monthly payment (V1) vs on final payment (V2)
 * - Non-compete: 6 months (V1) vs 12 months (V2)
 * - Term extension sentence: absent (V1) vs added (V2)
 * - Services feedback: "five (5) business days" (V1) vs "five business days" (V2, cosmetic)
 * - Confidentiality, liability, disputes, signatures: identical (unchanged anchors)
 *
 * Section ids mirror V2 exactly so deterministic alignment is by id.
 */
import type { FixtureSection } from "@/lib/documents/fixture";

export const FIXTURE_V1_ID = "sample-service-agreement-v1";

export const FIXTURE_V1_TITLE = "Independent Contractor Services Agreement";

export const FIXTURE_V1_SECTIONS: readonly FixtureSection[] = [
  {
    id: "sec-parties",
    documentId: "",
  ...
  },
  {
    ...
    paragraphs: [
      ...
      "The Client will provide timely feedback within five (5) business days of each delivery. Delays in feedback extend milestone dates by an equal period.",
    ],
  },
  {
    id: "sec-payment",
  ...
  },
  {
    ...
    clauseIds: [],
    paragraphs: [
      "Either party may terminate this Agreement for convenience with fifteen (15) days written notice to the other party.",
  ...
  },
  {
    ...
    clauseIds: [],
    paragraphs: [
      "The Contractor assigns to the Client all right, title, and interest in the final deliverables on receipt of each monthly payment in full.",
      "Until each payment, the Contractor retains ownership of all work product. The Contractor retains ownership of pre-existing methods, templates, and tools, granting the Client a non-exclusive license to use them as embedded in the deliverables.",
    ],
  },
  {
    id: "sec-restrictions",
...
export function getFixtureV1Sections(documentId: string): readonly FixtureSection[] {
  return FIXTURE_V1_SECTIONS.map((section) => ({ ...section, documentId }));
}
