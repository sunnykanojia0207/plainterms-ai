...
describe("Review Guide prompt builder", () => {
  it("versions the request and reuses validated findings", () => {
    const prompt = buildReviewGuidePrompt({ ...baseInput() });
    expect(prompt.version).toBe(AI_CONFIG.promptVersions.reviewGuide);
    expect(prompt.version).toBe("review-guide-v1");
  ...
  it("prioritizes comparison findings when provided", () => {
    const prompt = buildReviewGuidePrompt({
      ...
      silence: [],
      compareTitle: "Agreement v1",
    });
    expect(prompt.userPrompt).toContain('COMPARISON WITH "Agreement v1"');
    expect(prompt.userPrompt).toContain("Net 15 became net 30.");
  });

  it("states document-only preparation when no comparison exists", () => {
    const prompt = buildReviewGuidePrompt({ ...baseInput() });
    expect(prompt.userPrompt).toContain("No comparison available");
  });

  it("grounds new items in delimited sections with defenses", () => {
    const prompt = buildReviewGuidePrompt({ ...baseInput() });
    ...
    expect(prompt.userPrompt).toMatch(/all six areas/i);
  });
  ...
  it("keeps injected section text as data under defense rules", () => {
    ...
    const prompt = buildReviewGuidePrompt({ ...baseInput(), content: poisoned });
